...
led_tv = TimedLoad(60, time(hour=20), 3600, pd.DateOffset())
stove = TimedLoad(5250, time(hour=11), 7200, pd.DateOffset())

dishwasher = StaggeredLoad(900, time(hour=3), 9000, time_delta=pd.DateOffset())
washing_machine1 = StaggeredLoad(1000, time(hour=20), 5400, time_delta=pd.DateOffset())
tumble_drier = StaggeredLoad(power_consumption=2600,
                             original_start_time=time(hour=5),
                             cycle_duration=2700,
                             )

loads = [dishwasher, fridge, freezer, led_tv, washing_machine1]
